...
- The translation cache now holds 50,000 entries rather than 10,000, so far fewer phrases have to be translated a second time. It is kept in `translation_cache.jsonl`, one entry to a line, and a new translation is added to the end of the file instead of the whole file being written out again: writing no longer costs more the fuller the cache is, which is what kept it small. Polyglot rewrites the file only once it holds about twice as many lines as there are entries. A cache from an earlier version is taken over the first time this version starts, and the old `translation_cache.json` is then deleted.
